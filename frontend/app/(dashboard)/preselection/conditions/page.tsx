...
import { mockPreselectionCriteria, studyBranches, mockBranchConfigurations } from "@/lib/mock-data"
...
export default function ConditionsManagementPage() {
  ...
  useEffect(() => {
    ...
    router.replace(`/preselection/conditions?${params.toString()}`, { scroll: false })
  ...
  const filteredCriteria = selectedBranch === "all" ? criteria : criteria.filter((c) => c.branchId === selectedBranch)

  const searchedCriteria = filteredCriteria.filter((c) => {
    const branchName = studyBranches.find((b) => b.id === c.branchId)?.name || ""
    const searchLower = searchQuery.toLowerCase()
    return (
      branchName.toLowerCase().includes(searchLower) ||
      c.type.toLowerCase().includes(searchLower) ||
      (c.type === "subject" && c.subjectName?.toLowerCase().includes(searchLower)) ||
      (c.type === "mention" && c.minimumMention?.toLowerCase().includes(searchLower))
    )
  })

  const sortedCriteria = [...searchedCriteria].sort((a, b) => {
    const branchA = studyBranches.find((b) => b.id === a.branchId)?.name || ""
    const branchB = studyBranches.find((b) => b.id === b.branchId)?.name || ""
  ...
  const selectedBranchConfig =
    selectedBranch !== "all" ? mockBranchConfigurations.find((config) => config.id === selectedBranch) : null
        ...
        <div className="mx-auto max-w-4xl space-y-6">
              ...
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
                <div>
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">
                    Configure admission capacity and overflow actions for all branches
                  </p>
                </div>
          ...
          {selectedBranchConfig && (
            <Card className="border-primary/20 bg-primary/5">
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
                <div>
                  <CardTitle className="text-base">{selectedBranchConfig.branchName} - Branch Conditions</CardTitle>
                  <p className="mt-1 text-sm text-muted-foreground">
                ...
                <div className="flex gap-2">
                  <Button variant="default" size="sm" asChild>
                    <Link href={`/preselection/conditions/new?branchId=${selectedBranchConfig.id}`}>
                      <Plus className="mr-2 h-4 w-4" />
                      Add Criterion
                    </Link>
                  </Button>
                  <Button variant="outline" size="sm" asChild>
                    <Link href={`/preselection/branches/${selectedBranchConfig.id}/edit`}>
              ...
              <CardContent>
                <div className="grid gap-4 md:grid-cols-2">
                  <div className="flex items-center gap-3 rounded-lg border border-border bg-background p-3">
                    ...
                    <div>
                      <p className="text-xs text-muted-foreground">Admission Capacity</p>
                      <p className="text-xl font-bold text-foreground">{selectedBranchConfig.maxCapacity}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 rounded-lg border border-border bg-background p-3">
                    <div className="flex h-9 w-9 items-center justify-center rounded-full bg-primary/10">
                    ...
                    <div>
                      <p className="text-xs text-muted-foreground">Overflow Action</p>
                      <Badge
                        variant={
                          selectedBranchConfig.overflowAction === "reject"
                            ? "destructive"
          ...
          {sortedCriteria.length > 0 ? (
            sortedCriteria.map((criterion) => (
              <CriteriaCard key={criterion.id} criteria={criterion} onDelete={handleDelete} />
            ))
          ) : (
