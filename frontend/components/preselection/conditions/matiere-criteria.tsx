...
import type { CritereMatiere } from "@/types/preselections";

interface MatiereCriteriaProps {
  matieres: CritereMatiere[];
}

export function MatiereCriteria({
  matieres: initialMatieres,
}: MatiereCriteriaProps) {
  ...
  const addMatiere = () => {
    if (
      newMatiere.trim() &&
      !matieres.find((m) => m.matiere === newMatiere.trim())
    ) {
      setMatieres([
        ...matieres,
        {
  ...
  const updateNoteMinimale = (id: string, note: number | undefined) => {
    setMatieres(
      matieres.map((m) => (m.id === id ? { ...m, noteMinimale: note } : m))
    );
  };
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription className="mt-2">
              Définissez les matières considérées, leur ordre de priorité et une
              note minimale optionnelle.
            </CardDescription>
          </div>
        </div>
      </CardHeader>
          ...
          {matieres.length > 0 ? (
            <div className="space-y-2">
              {matieres.map((matiere, index) => {
                const hasSamePriorityAsPrevious =
                  index > 0 &&
                  matieres[index - 1].priorite === matiere.priorite;
                    ...
                    <div className="flex gap-2">
                      {index > 0 && (
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => setPrioriteEgale(matiere.id)}
                        >
                      ...
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => removeMatiere(matiere.id)}
                      >
